...
description: KirOpen's approach to creating technical designs that bridge requirements and implementation. Covers architecture, components, data models, interfaces, error handling, and decision documentation.
license: MIT
...
I use Mermaid syntax for all diagrams in design documents. They render natively in GitHub, VS Code, and most markdown viewers. Include diagrams when they clarify relationships that are hard to express in text — don't add them just for decoration.

Common diagram types:

**Architecture / component diagrams** (graph TD) for system structure:
```mermaid
graph TD
...
**State diagrams** for lifecycle and workflow flows:
```mermaid
stateDiagram-v2
    [*] --> Draft
    Draft --> Review
...
**ER diagrams** for data model relationships:
```mermaid
erDiagram
    USER ||--o{ ORDER : places
